...
public class RSLPStemmer {
	...
	public static void main(String[] args) {
		RSLPStemmer stemmer = new RSLPStemmer();
		
		String text = "@teste http://teste.com Temer se agarrará como pode às cadeiras de presidente. Sabe que pelos crimes que cometeu é de lá p cadeia, estilo Cunha #ForaTemer #SPDiretasJa".replaceAll("\n", "")
			      .replaceAll("rt\\s+", "")
			      .replaceAll("\\s+@\\w+", "")
			      .replaceAll("@\\w+", "")
			      .replaceAll("\\s+#\\w+", "")
	...
	public String stem(String term) {
		char [] termArray = term.trim().toCharArray();
		int newLength = this.stem(termArray, term.length());
		return new String(termArray).substring(0, newLength);
	}
